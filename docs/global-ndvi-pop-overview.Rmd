...
-   Scenario: set the NDVI of pixels in the bottom two tertiles to the NDVI value of the 83rd percentile (median of top tertile). In other words, only intervene upon pixels in the bottom two tertiles for that population category for that city for that biome. Note there are a few cities where biome varies within city.
...
Results are presented at the level of the global urban boundary here, but, following the methods above, they are first stratified by biome and are thus relative to biome within urban boundary if biome varies within urban boundary.
