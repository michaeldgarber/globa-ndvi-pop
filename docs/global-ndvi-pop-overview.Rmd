...
#Write this function. Copied directly from 
#scripts/analysis.R
...
```{r, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
setwd(here("data-processed"))
#city name lookup created scripts/read-gub.R
load("lookup_gub_city_name.RData")
load("lookup_city_id_country.RData")
hia_summary_gub_1mil_plus = pop_ndvi_gub_biome_tib %>% 
  ...
  left_join(lookup_city_id_country, by = "city_id") %>% 
  dplyr::select(
    city, country, starts_with("pop"),
    starts_with("ndvi"),starts_with("deaths")
  ) 
hia_summary_gub_1mil_plus %>% 
  ...
  knitr::kable(
    ...
    col.names = gsub("[_]", " ", names(hia_summary_gub_1mil_plus)),#remove underscores
    format.args = list(big.mark = ",", scientific = FALSE),
